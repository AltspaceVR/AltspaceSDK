...
describe('the layout utility', () => {
	...
	it('should be able position to enclosure', (done) => {
		...
		altspace.getEnclosure().then((enclosure) => {
			try {
				expect(container.position.x).to.equal(enclosure.innerWidth / 2 - 5);
			}
			catch (e) {
				done(e);
			}
	...
	it('throws an error for an invalid argument', (done) => {
		...
		scene.updateAllBehaviors();
		window.addEventListener('consoleevent', (evt) => {
			expect(evt.detail).to.match(/invalid layout position/);
			done();
		});
	});
