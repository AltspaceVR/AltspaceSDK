...
CursorEvents = function( scene, params ) {
	...
	if ( !scene || ! scene instanceof THREE.Scene ) {
		...
		console.log("Using old constructor: no scene, params: ", params);
		params = scene;
		scene = null;
	}
...
CursorEvents.prototype._createEventDetail = function( event ) {
	...
	// event.targetUuid is old format, event.target.uuid is new.
	// If event fired on scene, clear uuid which we expect to refer to an object.

	if ( event.target && !this.scene ) {
		// TODO: remove this once we depreciate old events
		console.error("Must pass scene to CursorEvent constructor when using new events.");
		return ; 
	}

	var detail = {
		targetUuid: event.target ?
			(event.target.uuid === this.scene.uuid ? null : event.target.uuid) :
			event.targetUuid,
		cursorRay: cursorRay,
	};
