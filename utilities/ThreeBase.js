...
window.ThreeBase = (function(){
	...
	function add(object3d, key, params) {
		validateAdd(object3d, key);
		...
		syncInstance.child(key).on('value', function(snapshot) {
			...
			if (data) {
				...
				// We are first to create this object, save it.
	...
	function validateAdd(object3d, key, params) {
		if (!syncInstance) {
		...
		if (!key) {
			die('ThreeBase: must specify unique key for object', object3d);
		}
		if (objectInfoForKey[key]) {
			die('ThreeBase: key '+ key +' already in use for another object');
		}
		if (params) {
